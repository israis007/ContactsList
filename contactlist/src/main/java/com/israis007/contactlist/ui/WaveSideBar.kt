...
class WaveSideBar @JvmOverloads constructor(
    ...
    private lateinit var mIndexItems: Array<String>
    ...
    init {
        context.withStyledAttributes(attrs, R.styleable.WaveSideBar, defStyleAttr, R.style.WaveSideBar){
            mDisplayMetrics = context.resources.displayMetrics

            val typedArray =
                context.obtainStyledAttributes(attrs, R.styleable.WaveSideBar)
            mLazyRespond =
                typedArray.getBoolean(R.styleable.WaveSideBar_sidebar_lazy_respond, false)
            mTextColor = typedArray.getColor(
                R.styleable.WaveSideBar_sidebar_text_color,
                Color.GRAY
            )
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        for (i in mIndexItems.indices){
            ...
            val baseLineX = when(mTextAlignment){
                TEXT_ALIGN_CENTER -> if (mSideBarPosition == POSITION_LEFT) paddingLeft + mBarWidth / 2 + mMaxOffset * scale    else    width - paddingRight - mBarWidth / 2 - mMaxOffset * scale
                TEXT_ALIGN_RIGHT -> if (mSideBarPosition == POSITION_LEFT)  paddingLeft + mBarWidth + mMaxOffset * scale        else    width - paddingRight - mMaxOffset * scale
                TEXT_ALIGN_LEFT -> if (mSideBarPosition == POSITION_LEFT)   paddingLeft + mMaxOffset * scale                    else    width - paddingRight - mBarWidth - mMaxOffset * scale
                else -> 0f
            }
